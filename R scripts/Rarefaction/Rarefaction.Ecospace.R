...
times<- 1000	# 100 takes about 3 minutes on my laptop

# Create subsets of data based on grouping variable of your choice ###########################
eco<- ecotemp<- subset(ecoall,ecoall$CLIM %in% c("temperate","Temperate")) 	# temperate only
eco<- ecotrop<- subset(ecoall,ecoall$CLIM %in% c("tropical","Tropical")) 	# tropical only
eco<- ecoclast<- subset(ecoall,ecoall$ENV %in% "siliciclastic") 			# clastic only
eco<- ecocarb<- subset(ecoall,ecoall$ENV %in% "carbonate") 				# carbonate only
